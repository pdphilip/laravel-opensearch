<?php

declare(strict_types=1);

namespace PDPhilip\OpenSearch\Schema\Definitions;

/**
 * @method $this type(string|array $value)
 * @method $this escapedTags(array $value)
 * @method $this mappings(array $value)
 * @method $this mappingsPath(string $value)
 * @method $this pattern(string $value)
 * @method $this replacement(string $value)
 * @method $this flags(string $value)
 */
class CharFilterPropertyDefinition extends FluentDefinitions {}
